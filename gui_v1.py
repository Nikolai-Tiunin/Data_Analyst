...
for env_key, env_val in thread_env_defaults.items():
...
log_dir = Path("logs_launch")
log_dir.mkdir(exist_ok=True)
log_file_path = log_dir / f"gui_v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
printt("Logging to %s", log_file_path.as_posix())
...
if __name__ == "__main__":
    ...
    class GUI:
        def __init__(self) -> None:
            ...
            self._input_silence_notified = False
            self._short_buffer_notified = False
            self.hostapis = None
            ...
            while True:
                ...
                if event == "threhold":
                    ...
                    if hasattr(self, "rvc"):
                        ...
                        if self.rvc.index_rate != self.gui_config.index_rate:
                            self.gui_config.index_rate = self.rvc.index_rate
                            if hasattr(self, "window"):
                                self.window["index_rate"].update(
                                    value=self.gui_config.index_rate
                                )
        ...
        def set_values(self, values):
            ...
            if index_rate <= 0 and os.path.isfile(self.gui_config.index_path):
                index_rate = 0.75
                printt(
                    "Index file detected but index rate was 0. Defaulting to %.2f.",
                    index_rate,
                )
                if hasattr(sg, "popup_auto_close"):
                    sg.popup_auto_close(
                        i18n("索引速率已自动调整为 %.2f") % index_rate,
                        auto_close_duration=2,
                        no_titlebar=True,
                    )
                if hasattr(self, "window"):
                    self.window["index_rate"].update(value=index_rate)
                values["index_rate"] = index_rate
        ...
        def start_vc(self):
            ...
            try:
                torch.set_num_threads(
                    max(1, min(os.cpu_count() or 1, int(self.gui_config.n_cpu)))
                )
            except Exception:
                pass
            ...
            if self.rvc.index_rate != self.gui_config.index_rate:
                printt(
                    "Index backend adjusted index_rate to %.2f",
                    self.rvc.index_rate,
                )
                self.gui_config.index_rate = self.rvc.index_rate
                if hasattr(self, "window"):
                    self.window["index_rate"].update(value=self.gui_config.index_rate)
            printt(
                "Stream init: sr=%d, block_frame=%d, crossfade=%d, index_rate=%.2f",
                self.gui_config.samplerate,
                self.block_frame,
                self.crossfade_frame,
                self.gui_config.index_rate,
        ...
        def audio_callback(
            ...
            try:
                if status:
                    printt("Stream status callback: %s", str(status))
                indata = librosa.to_mono(indata.T)
                input_peak = float(np.max(np.abs(indata))) if indata.size else 0.0
                if input_peak < 1e-4:
                    if not self._input_silence_notified:
                        printt(
                            "Warning: input block near silence (max=%.6f)", input_peak
                        )
                        self._input_silence_notified = True
                else:
                    if self._input_silence_notified:
                        printt(
                            "Input amplitude recovered (max=%.6f)", input_peak
                        )
                    self._input_silence_notified = False
                ...
                if infer_wav.shape[0] < self.block_frame:
                    if not self._short_buffer_notified:
                        printt(
                            "Warning: infer buffer shorter than block (%d < %d)",
                            infer_wav.shape[0],
                            self.block_frame,
                        )
                        self._short_buffer_notified = True
                else:
                    if self._short_buffer_notified:
                        printt(
                            "Infer buffer length recovered (%d samples)",
                            infer_wav.shape[0],
                        )
                    self._short_buffer_notified = False
                ...
                sola_scores = cor_nom[0, 0] / cor_den[0, 0]
                if sola_scores.ndim == 0:
                    sola_offset = 0
                elif sys.platform == "darwin":
                    # torch.argmax has limited MPS support; run on CPU for stability
                    sola_offset = int(torch.argmax(sola_scores.detach().cpu()).item())
                else:
                    sola_offset = int(torch.argmax(sola_scores).item())
                ...
                if infer_wav.numel() == 0:
                    ...
                    block_segment = infer_wav[: self.block_frame]
                    block_max = float(
                        torch.max(torch.abs(block_segment)).item()
                        if block_segment.numel() > 0
                        else 0.0
                    )
                if block_max < 1e-4:
                    if not self._silence_notified:
                        printt(
                            "Warning: output block near silence (max=%.6f)", block_max
                        )
                        self._silence_notified = True
                    out_block = np.zeros_like(outdata)
                else:
                    if self._silence_notified:
                        printt(
                            "Output amplitude recovered (max=%.6f)", block_max
                        )
                    self._silence_notified = False
                    out_block = (
                ...
                index_stats = getattr(self.rvc, "last_index_stats", {})
                printt(
                    "Infer time: %.2f (frames=%d, in_max=%.6f, out_max=%.6f, index_used=%s, index_time=%.3f)",
                    total_time,
                    frames,
                    input_peak,
                    block_max,
                    "Y" if index_stats.get("used") else "N",
                    index_stats.get("time", 0.0),
                )
            except Exception as exc:
                printt(
                    "Exception in audio_callback: %s\n%s",
                    str(exc),
